...
NSString *const WriteUUIDStirng = @"FFF2";
NSString *const NotifyUUIDStirng = @"FFF1";
...
//    [self.myCentralManager scanForPeripheralsWithServices:nil options:@{CBCentralManagerScanOptionAllowDuplicatesKey:@YES}];
    ...
    [_myPeripherals removeAllObjects];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    CBUUID *notifyUUID = [CBUUID UUIDWithString:NotifyUUIDStirng];
    CBUUID *writeUUID = [CBUUID UUIDWithString:WriteUUIDStirng];
    
    for(CBCharacteristic* c in service.characteristics){
        NSLog(@"特征 UUID: %@ (%@)", c.UUID.data, c.UUID);
        
        if([c.UUID isEqual:writeUUID]){
            self.writeCharacteristic = c;
            NSLog(@"找到WRITE : %@", c);
        }else if([c.UUID isEqual:notifyUUID]){
